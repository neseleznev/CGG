...
namespace Ellipse
{
	class Program
	{
		...
		private static void DrawPixelSsshifted(Bitmap image, int screenX, int screenY)
		{
			...
			if (InRange(XShift + Middle + screenX, 0, WindowSize) &&
				InRange(YShift + Middle - screenY, 0, WindowSize))
				image.SetPixel(XShift + Middle + screenX, YShift + Middle - screenY, Color.Blue);

			if (InRange(XShift + Middle + screenX, 0, WindowSize) &&
				InRange(YShift + WindowSize - Middle + screenY, 0, WindowSize))
		...
		private static void CreateImage(Bitmap image)
		{
			...
			int screenY = YCoef;
		    double etalonSum = YCoef > XCoef 
                ? SumDistToFocuses(0, YCoef, focus)
                : SumDistToFocuses(XCoef, 0, focus);
			DrawPixelSsshifted(image, 0, YCoef);
			for (int screenX = 0; screenX < XCoef && screenY > 0; )
			{
		...
		{
			var form = new Form { ClientSize = new Size(WindowSize, WindowSize) };
